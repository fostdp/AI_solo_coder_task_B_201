...
from .service import permeability_service
...
router = APIRouter(prefix="/api/permeability", tags=["Permeability Analysis"])
...
async def analyze_permeability_impact(
    ...
    return permeability_service.analyze_impact(
...
async def analyze_historical(casting_id: str):
    result = permeability_service.analyze_historical(casting_id)
    if "error" in result:
...
async def compare_scenarios(req: CompareScenariosRequest):
    ...
    return permeability_service.compare_scenarios(scenarios)
